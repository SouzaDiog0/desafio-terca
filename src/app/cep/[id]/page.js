...
export default function page({params}){
    ...
    useEffect(() => {
        ...
        .catch((error) => {
            console.log("deu erro essa porra:", error)
    ...
    if(data.erro || data.error) {
        return(
            <div className="flex flex-col justify-center h-screen bg-[#1f2937] text-white font-bold items-center gap-4">
                ...
                <Link href="/" className="bg-blue-700 px-8 py-3 rounded-lg font-bold text-white text-lg hover:bg-blue-800 transition-all duration-300 ease-in-out">
                    Buscar outro CEP
                </Link>
            </div>
        )
